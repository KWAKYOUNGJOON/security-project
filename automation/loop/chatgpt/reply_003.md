...
## WHY
Now that the CLI can use either the default loop root or an override, accidental re-initialization becomes a more realistic safety risk. Guarding `init` against silent overwrite is a high-value follow-up because it prevents state loss and operator confusion without changing the normal happy-path loop.